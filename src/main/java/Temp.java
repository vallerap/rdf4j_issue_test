...
public class Temp {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        ...
        String selectQuery = "PREFIX owl:<http://www.w3.org/2002/07/owl#> \n" +
                "                PREFIX xsd:<http://www.w3.org/2001/XMLSchema#> \n" +
                "                PREFIX freebase:<http://rdf.freebase.com/ns/> \n" +
                "                PREFIX skos:<http://www.w3.org/2004/02/skos/core#> \n" +
                "                PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> \n" +
        ...
        IRI unifiedSchemaGraph = SimpleValueFactory.getInstance().createIRI("tmp:ve/uni-schema");
        ...
        IRI skosSchemaGraph = SimpleValueFactory.getInstance().createIRI("tmp:ve/skos");
        String query = String.format("ADD SILENT <%s> TO <%s>", skosSchemaGraph, unifiedSchemaGraph);
        connection.prepareUpdate(QueryLanguage.SPARQL, query).execute();

        IRI rdfSchemaGraph = SimpleValueFactory.getInstance().createIRI("tmp:ve/rdf-schema");
        query = String.format("ADD SILENT <%s> TO <%s>", rdfSchemaGraph, unifiedSchemaGraph);
        connection.prepareUpdate(QueryLanguage.SPARQL, query).execute();


        connection = sailRepository.getConnection();
        connection.prepareTupleQuery(selectQuery).evaluate().hasNext();
    }
