...
public class Temp {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        ...
        /*Type STATEMENT_TYPE = new TypeToken<Collection<Object>>() {
        }.getType();

        URL resource = Temp.class.getResource("jsonObjectString.json");
        Paths.get(resource.toURI());

        JsonReader jr = new JsonReader(new FileReader(Paths.get(resource.toURI()).toFile()));

        Collection<LinkedTreeMap> objects = new Gson().fromJson(jr, STATEMENT_TYPE);


        ArrayList<Statement> statements = new ArrayList<>();

        for (LinkedTreeMap i : objects){

        }*/

        Collection<Statement> statements = getStatements("objects.rdf");
        ...
        // connection.prepareTupleQuery(query2).evaluate().hasNext();

        SimpleValueFactory vF = SimpleValueFactory.getInstance();
        
        IRI unifiedSchemaGraph = vF.createIRI("tmp:validationengine/uni-schema");

       // connection.prepareUpdate(String.format("CREATE GRAPH <%s>", unifiedSchemaGraph)).execute();


        //connection.prepareTupleQuery(query2).evaluate().hasNext();
        //connection.prepareTupleQuery(query2).evaluate().hasNext();
        add(statements, connection, unifiedSchemaGraph);
        connection = sailRepository.getConnection();
        add(getStatements("/skos.rdf"), connection, vF.createIRI("tmp:validationengine/skos"));
        connection = sailRepository.getConnection();

        add(getStatements("/skos-xl.rdf"), connection, vF.createIRI("tmp:validationengine/skos"));
        connection = sailRepository.getConnection();

        //add(getStatements("/skos-xl.rdf"), connection, vF.createIRI("tmp:validationengine/skos"));
        add(getStatements("/pp-schema.ttl"), connection, vF.createIRI("tmp:validationengine/pp-schema"));
        connection = sailRepository.getConnection();

        add(getStatements("/rdfs.ttl"), connection, vF.createIRI("tmp:validationengine/rdf-schema"));
        connection = sailRepository.getConnection();

        add(getStatements("/fail.ttl"), connection, vF.createIRI("http://test.com/inportdata"));
        connection = sailRepository.getConnection();
        // add(statements,connection,null);
        // connection = sailRepository.getConnection();


        IRI skosSchemaGraph = SimpleValueFactory.getInstance().createIRI("tmp:validationengine/skos");
        String query = String.format("ADD <%s> TO <%s>", skosSchemaGraph, unifiedSchemaGraph);
        connection.prepareUpdate(QueryLanguage.SPARQL, query).execute();

        IRI rdfSchemaGraph = SimpleValueFactory.getInstance().createIRI("tmp:validationengine/rdf-schema");
        query = String.format("ADD <%s> TO <%s>", rdfSchemaGraph, unifiedSchemaGraph);
        connection.prepareUpdate(QueryLanguage.SPARQL, query).execute();
    ...
    public static void add(Collection<Statement> statements, RepositoryConnection repoConnection, Resource... graphs) {
        if (graphs == null || graphs.length == 0 || graphs[0] == null) {
            graphs = new Resource[]{SimpleValueFactory.getInstance().createIRI("http://test.com/inportdata")};
        }
